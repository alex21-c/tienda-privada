...
// ==================== PROCESAR COLOR Y TALLAS CORRECTAMENTE ====================
function procesarColorConTallas(producto) {
    if (producto.colores && producto.colores.length > 0 && producto.variantes && producto.variantes !== '') {
        const variantesTexto = producto.variantes;
        
        // Verificar si las variantes ya tienen el formato con color
        const tieneFormatoColor = variantesTexto.includes(':') && /^[A-ZÑa-zñ]+:/i.test(variantesTexto);
        
        if (!tieneFormatoColor) {
            ...
            if (primerColor) {
                ...
                primerColor.tallas = tallasLista;
                producto.colores[0] = primerColor;
                producto.variantes = '';
            }
        }
    }
...
function mostrarProductos() {
    ...
    contenedor.innerHTML = filtrados.map(p => `
        <div class="producto-card" onclick="mostrarModal(${p.id})">
            ...
            <h3 class="producto-titulo">${p.nombre.substring(0, 60)}</h3>
            <div class="producto-precio">$${p.precio.toFixed(2)}</div>
            <button class="btn-agregar" onclick="event.stopPropagation(); agregarAlCarrito(${p.id})">➕ Agregar al carrito</button>
        </div>
    `).join('');
}

function getIconoGenero(g) { 
    const iconos = { hombre:'👨', mujer:'👩', niño:'👦', niña:'👧', unisex:'👥' };
    return iconos[g] || '👤'; 
}
...
function mostrarModal(id) {
    ...
    if (modalImagen) {
        modalImagen.src = (producto.colores && producto.colores[0]) ? producto.colores[0].imagen : producto.imagen;
    }
    ...
    if (modalGenero) modalGenero.textContent = `${getIconoGenero(producto.genero)} ${producto.genero || 'Unisex'}`;
    if (modalPrecio) modalPrecio.textContent = `$${producto.precio.toFixed(2)}`;
    if (modalDescripcion) modalDescripcion.textContent = producto.descripcion || `Producto de alta calidad de ${producto.origen}.`;
...
function mostrarSelectoresColor(colores) {
    ...
    if (colores.length > 0) {
        ...
        if (colores[0].tallas && colores[0].tallas.length > 0) {
...
function mostrarTallas(tallas) {
    ...
    if (tallas && tallas.length > 0 && tallas[0].talla && !isNaN(tallas[0].precio)) {
        variantesDiv.style.display = 'block';
        selectorVariante.innerHTML = '<option value="">Selecciona una talla</option>';
        
        tallas.forEach(t => {
            const precioValido = typeof t.precio === 'number' ? t.precio : parseFloat(t.precio);
            if (!isNaN(precioValido)) {
                const option = document.createElement('option');
                option.value = precioValido;
                option.setAttribute('data-talla', t.talla);
                option.textContent = `${t.talla} - $${precioValido.toFixed(2)}`;
...
function mostrarTallasDirectas(variantes) {
    ...
    if (variantes && variantes.length > 0) {
        ...
        const tallasLista = Array.isArray(variantes) ? variantes : variantes.split(',').map(t => {
            const [talla, precio] = t.split(':');
            return { talla: talla?.trim(), precio: parseFloat(precio) };
        });
        
        tallasLista.forEach(t => {
            if (t.talla && !isNaN(t.precio)) {
                const option = document.createElement('option');
...
function actualizarCarrito() {
    ...
    carrito.forEach(item => {
        ...
        itemDiv.innerHTML = `
            ...
            <button class="btn-eliminar" onclick="eliminarItemCarrito(${item.id}, '${item.nombre.replace(/'/g, "\\'")}')">🗑️</button>
        `;
...
function eliminarItemCarrito(id, nombre) {
        ...
        if (carrito.length === 0) {
            ...
            carrito.forEach(item => {
                ...
                itemDiv.innerHTML = `
                    ...
                    <button class="btn-eliminar" onclick="eliminarItemCarritoPanel(${item.id}, '${item.nombre.replace(/'/g, "\\'")}')">🗑️</button>
                `;
...
function eliminarItemCarritoPanel(id, nombre) {
    carrito = carrito.filter(item => !(item.id == id && item.nombre == nombre));
    guardarCarrito();
    actualizarCarritoTop();
    actualizarCarrito();
...
async function enviarPedidoModal() {
    ...
    carrito.forEach(item => {
        ...
        detalleProductos += `${item.nombre} x${item.cantidad} - $${subtotal.toFixed(2)}\n`;
    });
...
function mostrarFactura(pedido) {
    ...
    facturaDiv.innerHTML = `
        ...
        <div class="factura-envio-nota">
            ⚠️ Los costos de envío ($${3.50} por libra + $${1.00} delivery) se pagan en Panamá.
        </div>
    ...
    <head>
        ...
        <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            .producto-linea { padding: 5px 0; border-bottom: 1px solid #eee; }
...
function configurarZoomEnModalProducto() {
    setTimeout(() => {
        ...
        if (modalImagen) {
            ...
            const clone = modalImagen.cloneNode(true);
            modalImagen.parentNode?.replaceChild(clone, modalImagen);
            const nuevaImagen = document.getElementById('modal-imagen');
            if (nuevaImagen) {
                nuevaImagen.style.cursor = 'zoom-in';
                nuevaImagen.onclick = (e) => {
                    e.stopPropagation();
                    window.abrirZoomImagen?.(nuevaImagen.src);
                };
            }
        }
    }, 150);
...
function configurarZoomModal() {
    ...
    if (zoomModal) {
        zoomModal.addEventListener('click', (e) => {
            if (e.target === zoomModal) window.cerrarZoomImagen();
        });
    }
    
    if (zoomCerrar) zoomCerrar.addEventListener('click', window.cerrarZoomImagen);
    
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') window.cerrarZoomImagen();
    });
}
