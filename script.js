...
async function cargarProductos() {
    ...
    try {
        ...
        productos = datos;
        if (!productos.length) productosDiv.innerHTML = '<p class="loading">📦 No hay productos</p>';
        else mostrarProductos();
    } catch (error) {
        console.error('Error:', error);
        if (productosDiv) productosDiv.innerHTML = '<p class="loading">❌ Error de conexión</p>';
    }
}
...
function mostrarProductos() {
    ...
    if (!filtrados.length) { contenedor.innerHTML = '<p class="loading">No hay productos</p>'; return; }
    contenedor.innerHTML = filtrados.map(p => `
        <div class="producto-card" onclick="mostrarModal(${p.id})">
            ...
            <div class="producto-origen origen-${(p.origen || 'temu').toLowerCase()}">${p.origen}</div>
...
function mostrarPresentaciones(texto) {
    ...
    if (matches.length) {
        ...
        sel.innerHTML = '<option value="">Selecciona una opción</option>' + matches.map(m => `<option value="${m[3]}" data-cantidad="${m[1]}" data-texto="${m[1]} ${m[2]}">${m[1]} ${m[2]} - $${parseFloat(m[3]).toFixed(2)}</option>`).join('');
        sel.onchange = () => {
            presentacionSeleccionada = sel.value ? { cantidad: parseInt(sel.selectedOptions[0].dataset.cantidad), texto: sel.selectedOptions[0].dataset.texto, precio: parseFloat(sel.value) } : null;
...
function mostrarModal(id) {
    ...
    document.getElementById('modal-origen').className = `origen-${(producto.origen || 'temu').toLowerCase()}`;
    ...
    if (producto.colores?.length) {
        ...
        else document.getElementById('modal-variantes').style.display = 'none';
    }
...
function mostrarSelectoresColor(colores) {
    ...
    container.innerHTML = colores.map((c, i) => `<button style="padding:8px 20px;border:2px solid ${i===0?'#00D4FF':'#ccc'};background:${i===0?'#00D4FF':'white'};border-radius:40px;cursor:pointer">${c.nombre}</button>`).join('');
    document.querySelectorAll('#selector-color button').forEach((btn, i) => {
        btn.onclick = () => {
            document.querySelectorAll('#selector-color button').forEach(b => { b.style.border = '2px solid #ccc'; b.style.background = 'white'; b.style.color = '#333'; });
            btn.style.border = '2px solid #00D4FF'; btn.style.background = '#00D4FF'; btn.style.color = 'white';
            colorSeleccionado = colores[i];
            document.getElementById('modal-imagen').src = colores[i].imagen;
            document.getElementById('modal-precio').textContent = `$${colores[i].precio.toFixed(2)}`;
            let tallas = colores[i].tallas;
            if (!tallas?.length && productoActual.variantes && !productoActual.variantes.includes(colores[i].nombre)) {
                tallas = productoActual.variantes.split(',').map(t => { const [talla, p] = t.split(':'); return { talla, precio: parseFloat(p) }; });
            }
    ...
    if (!tallasIniciales?.length && productoActual.variantes && !productoActual.variantes.includes(colores[0].nombre)) {
        tallasIniciales = productoActual.variantes.split(',').map(t => { const [talla, p] = t.split(':'); return { talla, precio: parseFloat(p) }; });
    }
...
function mostrarTallas(tallas) {
    ...
    div.style.display = 'block';
    sel.innerHTML = '<option value="">Selecciona una talla</option>' + tallas.map(t => `<option value="${t.precio}" data-talla="${t.talla}">${t.talla} - $${t.precio.toFixed(2)}</option>`).join('');
    sel.onchange = () => {
...
function mostrarTallasDirectas(variantes) {
    const tallas = Array.isArray(variantes) ? variantes : variantes?.split(',').map(t => { const [talla, precio] = t.split(':'); return { talla, precio: parseFloat(precio) }; });
    mostrarTallas(tallas);
}
...
function actualizarCarrito() {
    ...
    carrito.forEach(item => {
        ...
        total += subtotal;
        lista.innerHTML += `<div class="item-carrito"><div><strong>${item.nombre}</strong> (${item.origen})<br>$${item.precio.toFixed(2)} x ${item.cantidad} = $${subtotal.toFixed(2)}</div><button class="btn-eliminar" onclick="eliminarItem(${item.id}, '${item.nombre.replace(/'/g, "\\'")}')">🗑️</button></div>`;
    });
    if (totalSpan) totalSpan.textContent = total.toFixed(2);
}

function eliminarItem(id, nombre) {
    carrito = carrito.filter(i => !(i.id == id && i.nombre == nombre));
    guardarCarrito();
    actualizarCarrito();
}
...
function actualizarCarritoTop() {
    ...
    if (totalPanel) totalPanel.textContent = total.toFixed(2);
    if (listaPanel) {
        if (!carrito.length) listaPanel.innerHTML = '<div class="vacio">✨ Tu carrito está vacío</div>';
        else listaPanel.innerHTML = carrito.map(i => `<div class="item-carrito"><div><strong>${i.nombre}</strong><br>$${i.precio.toFixed(2)} x ${i.cantidad} = $${(i.precio*i.cantidad).toFixed(2)}</div><button class="btn-eliminar" onclick="eliminarItem(${i.id}, '${i.nombre.replace(/'/g, "\\'")}')">🗑️</button></div>`).join('');
    }
}

function toggleCarrito() { document.getElementById('carrito-panel')?.classList.toggle('open'); document.getElementById('overlay')?.classList.toggle('active'); }
function cerrarCarrito() { document.getElementById('carrito-panel')?.classList.remove('open'); document.getElementById('overlay')?.classList.remove('active'); }
...
async function enviarPedidoModal() {
    ...
    carrito.forEach(i => { let subtotal = i.precio * i.cantidad; total += subtotal; productosTexto += `${i.nombre} (${i.origen}) x${i.cantidad} - $${subtotal.toFixed(2)}\n`; });
    const pedido = { nombre, telefono, direccion, metodoPago, estadoPago: metodoPago === 'Yappy' ? 'Pendiente de pago' : 'Pendiente', productos: productosTexto, totalProductos: total, envioLibra: 3.50, delivery: 1.00, envioTotal: 4.50, idPedido: Date.now().toString() };
    await guardarPedido(pedido);
    cerrarModalPago(); cerrarCarrito();
...
async function verificarPagoYGenerarFactura() {
    ...
    try {
        ...
        cerrarConfirmarPago();
        mostrarFactura({ nombre: pedido.nombre, telefono: pedido.telefono, direccion: pedido.direccion, metodoPago: pedido.metodoPago, productos: pedido.productos, totalProductos: pedido.total, envioLibra: 3.50, delivery: 1.00, idPedido: pedido.idPedido });
...
async function buscarPedidosPorNombreModal() {
    ...
    if (!nombre) return resultadoDiv.innerHTML = '<p class="error">⚠️ Ingresa tu nombre</p>';
    resultadoDiv.innerHTML = '<p>Cargando...</p>';
    try {
        let res = await fetch(`${API_URL}?action=getPedidosByNombre&nombre=${encodeURIComponent(nombre)}`);
        let pedidos = await res.json();
        if (!pedidos.length) return resultadoDiv.innerHTML = '<p class="vacio">📭 No hay pedidos</p>';
        resultadoDiv.innerHTML = pedidos.map(p => `<div class="pedido-card"><div class="pedido-header"><span>#${p.idPedido || p.id}</span><span>${new Date(p.fecha).toLocaleDateString()}</span><span class="estado-${p.estado}">${p.estado}</span></div><div class="pedido-productos">${p.productos?.replace(/\n/g,'<br>')}</div><div class="pedido-total">Total: $${(p.total||0).toFixed(2)}</div></div>`).join('');
    } catch(e) { resultadoDiv.innerHTML = '<p class="error">❌ Error</p>'; }
}
...
function configurarFiltros() {
    ...
    document.querySelectorAll('.filtro-genero-btn').forEach(btn => btn.addEventListener('click', () => {
        document.querySelectorAll('.filtro-genero-btn').forEach(b => b.classList.remove('active'));
        btn.classList.add('active');
...
function mostrarEnlaceAdmin(enlace) {
    ...
    if (esAdmin && enlace) {
        ...
        a.href = enlace; a.textContent = enlace;
...
function configurarZoomEnModalProducto() {
    setTimeout(() => {
        ...
        if (img) {
            ...
            clone.onclick = (e) => { e.stopPropagation(); window.abrirZoomImagen?.(clone.src); };
        }
    }, 150);
}
...
function configurarModales() {
    ...
    modals.forEach(id => { let modal = document.getElementById(id); if(modal) modal.onclick = e => { if(e.target === modal) modal.style.display = 'none'; }; });
